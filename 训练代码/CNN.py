...
def cnn_train():
    ...
    # 模型训练
    print("开始训练cnn")
    history=model.fit(X_train, y_train, epochs=100,validation_split=0.05)  # 总loss为7个loss的和
    model.save('cnn.h5')
...
Lic_img=cv2.imread("4.jpg")
imGray=exchange(Lic_img)
#imGray = cv2.cvtColor(Lic_img, cv2.COLOR_BGR2GRAY)
ret,imGray = cv2.threshold(imGray,100,255,cv2.THRESH_BINARY)
img = cv2.cvtColor(imGray, cv2.COLOR_GRAY2RGB)
cv2.imshow('a',img)
cv2.waitKey()
Lic_img=img.reshape(1, 80, 240, 3)
